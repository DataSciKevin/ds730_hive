...
CREATE EXTERNAL TABLE IF NOT EXISTS master(id STRING, byear INT, bmonth INT, bday INT, bcountry STRING, bstate STRING, bcity STRING, dyear INT, dmonth INT, dday INT, dcountry STRING, dstate STRING, dcity STRING, fname STRING, lname STRING, name STRING, weight INT, height INT, bats STRING, throws STRING, debut STRING, finalgame STRING, retro STRING, bbref STRING) ROW FORMAT DELIMITED FIELDS TERMINATED BY ',' LOCATION '/user/maria_dev/hivetest/master';
 ...
 select rankdobid
from (select mstr.dobid as rankdobid, DENSE_RANK() OVER(ORDER BY mstr.dobcount ) as rankval 
	  from
		(select CONCAT(master.bmonth,'/',master.bstate) as dobid, sum(batting.hits / batting.ab) as dobcount 
 		from master, batting
 		where master.id = batting.id and 
		 master.bmonth is not null 
		 and master.bstate is not null
		 and batting.hits >= 0 
		 and batting.ab >= 0
 	  group by CONCAT(master.bmonth,'/',master.bstate) 
	  having count(batting.id) >= 5 and sum(batting.hits) > 100 ) mstr 
 ) rankmaster
 where rankval = 1;